...
// It fails the test if the retrieved value doesn't match the expected value.
...
// TestBasicFail tests basic primary/backup functionality including:
// - Single primary operation
// - Adding a backup
// - Primary failure and backup promotion
// - New server not becoming active without proper initialization
func TestBasicFail(t *testing.T) {
	...
	// Test basic operations with single primary
	ck.Put("111", "v1")
	check(ck, "111", "v1")
	...
	// Test primary failure and backup promotion
	...
	// Test that new server doesn't become active without proper initialization
...
// TestAtMostOnce tests at-most-once semantics with unreliable network conditions.
// It verifies that PutHash operations maintain consistency even with network failures.
func TestAtMostOnce(t *testing.T) {
	...
	// Wait for primary and backup to be established
	...
	// Give primary and backup time to acknowledge and initialize
	...
	// Perform 100 PutHash operations to test at-most-once semantics
	for i := 0; i < 100; i++ {
		v := strconv.Itoa(i)
		pv := ck.PutHash(k, v)
		if pv != val {
...
// TestFailPut tests Put operations immediately after backup and primary failures.
// It verifies that the system continues to operate correctly during failover scenarios.
...
// TestConcurrentSame tests concurrent Put operations on the same key.
// It verifies that primary and backup servers process operations in the same order
// and maintain identical state, ensuring consistency under concurrent load.
...
// TestRepeatedCrash tests the system's resilience under constant server crashes and restarts.
// It performs continuous Put/Get operations while randomly killing and restarting servers
// to verify the system maintains consistency and availability.
