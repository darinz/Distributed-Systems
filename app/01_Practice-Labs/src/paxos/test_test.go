// Package paxos_test contains comprehensive tests for the Paxos consensus algorithm implementation.
// These tests verify the correctness of the Paxos protocol under various conditions including
// network failures, partitions, concurrent proposals, and memory management.
package paxos
...
// ndecided counts how many peers have decided on the given sequence number.
// It also verifies that all decided peers agree on the same value.
...
// waitn waits for at least 'wanted' peers to decide on the given sequence number.
...
// waitmajority waits for a majority of peers to decide on the given sequence number.
func waitmajority(t *testing.T, pxa []*Paxos, seq int) {
	waitn(t, pxa, seq, (len(pxa)/2)+1)
}

// checkmax verifies that no more than 'max' peers have decided on the given sequence number.
...
// cleanup kills all Paxos peers in the given slice.
...
// noTestSpeed is a performance test that measures the time to reach 20 agreements.
// This test is disabled by default (prefixed with "no") but can be enabled for performance testing.
func noTestSpeed(t *testing.T) {
	...
	for i := 0; i < npaxos; i++ {
		pxa[i] = Make(pxh, i, nil, false, "", false)
	}
...
// TestBasic tests fundamental Paxos functionality including:
// - Single proposer agreement
// - Multiple proposers with the same value
// - Multiple proposers with different values
// - Out-of-order instance processing
...
// TestDeaf tests Paxos behavior when some peers become unreachable (deaf).
// It verifies that deaf peers don't learn about decisions they shouldn't.
...
func TestOld(t *testing.T) {
	...
	pxa[1] = Make(pxh, 1, nil, false, "", false)
	pxa[2] = Make(pxh, 2, nil, false, "", false)
	pxa[3] = Make(pxh, 3, nil, false, "", false)
	pxa[1].Start(1, 111)

	waitmajority(t, pxa, 1)

	pxa[0] = Make(pxh, 0, nil, false, "", false)
	pxa[0].Start(1, 222)

	waitn(t, pxa, 1, 4)

	if false {
		pxa[4] = Make(pxh, 4, nil, false, "", false)
		waitn(t, pxa, 1, npaxos)
	}
