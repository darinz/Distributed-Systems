...
// ShardKV server: a replica in a group that serves a subset of shards.
// Each group orders operations (Get/Put/Reconfigure) via Paxos to ensure
// linearizable application of client requests and configuration changes.
...
// Op is the replicated operation that ShardKV agrees upon using Paxos.
// Args holds the concrete request type for the operation.
type Op struct {
...
// ShardKV is a single server in a replica group for the sharded KV store.
// Each group uses Paxos to order operations and reconfigurations.
type ShardKV struct {
    mu         sync.Mutex
    l          net.Listener
    me         int
...
// InquireArgs requests the state of a shard from another group during reconfig.
type InquireArgs struct {
  Shard  int
  Config shardmaster.Config
}

// InquireReply returns shard state and client deduplication metadata.
...
// ReconfigArgs bundles a target configuration with state obtained via inquiries.
type ReconfigArgs struct {
    Config  shardmaster.Config
    Inquiry InquireReply
}

// Get handles a client Get request by proposing it via Paxos and returning the value.
...
// Put handles a client Put/PutHash request by proposing it via Paxos.
...
// Reconfigure transitions the server to the supplied configuration by fetching
// state for shards that move into this group, then committing the change via Paxos.
...
// waitForPaxos waits until Paxos has decided a value for the sequence number
// and returns the chosen Op.
...
// status checks whether the supplied op has already been processed or should be rejected.
func (kv *ShardKV) status(op Op) (Err, string) {
    ...
    case Get, Put:
        ...
        if shard := key2shard(key); kv.gid != kv.config.Shards[shard] {
            ...
            // we already have a response for this client
...
// execute proposes the op via Paxos, waits for it to be chosen, and applies the effect.
