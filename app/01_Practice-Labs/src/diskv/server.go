...
// Op is the replicated operation recorded via Paxos.
// Args carries the concrete request payload for the operation.
type Op struct {
...
// DisKV is a persistent shard-serving replica using Paxos for operation ordering.
// The server persists KV data and relevant metadata to survive restarts.
type DisKV struct {
	mu         sync.Mutex
	l          net.Listener
	me         int
