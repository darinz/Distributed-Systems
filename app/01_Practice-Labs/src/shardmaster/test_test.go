// Package shardmaster_test contains comprehensive tests for the shard master service.
// These tests verify the correctness of the shard master under various conditions
// including group joins/leaves, load balancing, and fault tolerance.
package shardmaster
...
// It creates a path based on the user ID, process ID, tag, and host number.
...
// cleanup kills all ShardMaster servers in the given slice.
...
// check verifies that the shard master configuration is correct.
// It checks that the expected groups exist, all shards are assigned to valid groups,
// and the shard distribution is balanced.
func check(t *testing.T, groups []int64, ck *Clerk) {
	...
	// Check that all shards are assigned to valid groups
	if len(groups) > 0 {
	...
	// Check that shard distribution is balanced
...
// TestBasic tests fundamental shard master functionality including:
...
func TestBasic(t *testing.T) {
	...
	// Store configurations for historical query testing
	cfa := make([]Config, 6)
	cfa[0] = ck.Query(-1)

	// Test initial state (no groups)
	check(t, []int64{}, ck)

	// Test joining first group
	gid1 := int64(1)
	ck.Join(gid1, []string{"x", "y", "z"})
	check(t, []int64{gid1}, ck)
	cfa[1] = ck.Query(-1)

	// Test joining second group
	gid2 := int64(2)
	ck.Join(gid2, []string{"a", "b", "c"})
	check(t, []int64{gid1, gid2}, ck)
	cfa[2] = ck.Query(-1)

	// Test joining same group again (should be idempotent)
	ck.Join(gid2, []string{"a", "b", "c"})
	check(t, []int64{gid1, gid2}, ck)
	cfa[3] = ck.Query(-1)

	// Verify server assignments are correct
	cfx := ck.Query(-1)
	sa1 := cfx.Groups[gid1]
	if len(sa1) != 3 || sa1[0] != "x" || sa1[1] != "y" || sa1[2] != "z" {
		t.Fatalf("wrong servers for gid %v: %v", gid1, sa1)
	}
	sa2 := cfx.Groups[gid2]
	if len(sa2) != 3 || sa2[0] != "a" || sa2[1] != "b" || sa2[2] != "c" {
		t.Fatalf("wrong servers for gid %v: %v", gid2, sa2)
	}

	// Test leaving first group
	ck.Leave(gid1)
	check(t, []int64{gid2}, ck)
	cfa[4] = ck.Query(-1)

	// Test leaving same group again (should be idempotent)
	ck.Leave(gid1)
	check(t, []int64{gid2}, ck)
	cfa[5] = ck.Query(-1)
	...
	// Test that historical configurations can be retrieved correctly
	for i := 0; i < len(cfa); i++ {
	...
	// Test that Paxos Min() advances after operations
	for i, sm := range sma {
		if sm.px.Min() <= 0 {
	...
	// Test that joins result in minimal shard transfers
	c1 := ck.Query(-1)
	for i := 0; i < 5; i++ {
	...
	// Test that leaves result in minimal shard transfers
	for i := 0; i < 5; i++ {
...
// TestUnreliable tests the shard master under unreliable network conditions.
// It verifies that the system can handle network failures and server crashes.
func TestUnreliable(t *testing.T) {
	...
	// Test concurrent operations with server failure
	const npara = 20
	gids := make([]int64, npara)
	var ca [npara]chan bool
	for xi := 0; xi < npara; xi++ {
...
// TestFreshQuery tests that Query(-1) returns the latest configuration
// even when some servers are temporarily unavailable.
func TestFreshQuery(t *testing.T) {
	...
	// Temporarily rename server 0's socket to simulate unavailability
	...
	// Perform a join operation on server 1
	ck1.Join(1001, []string{"a", "b", "c"})
	
	// Query from server 0 should still get the latest configuration
	c := ck0.Query(-1)
	_, ok := c.Groups[1001]
	if !ok {
